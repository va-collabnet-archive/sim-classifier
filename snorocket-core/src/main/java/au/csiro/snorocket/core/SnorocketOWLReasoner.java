...
public class SnorocketOWLReasoner implements OWLReasoner {
	...
	/**
	 * TODO: deal with ReasonerInterruptedException and TimeOutException.
	 * TODO: is this version of Snorocket using the problem list to report
	 * inconsistencies?
	 */
	@Override
	...
	public boolean isSatisfiable(OWLClassExpression classExpression)
		...
		if (classExpression.isAnonymous()) {
			...
			int c = factory.getConcept(
					classExpression.asOWLClass().toStringID());
	...
	public Node<OWLObjectPropertyExpression> getTopObjectPropertyNode() {
		return new OWLObjectPropertyNode(
				owlFactory.getOWLTopObjectProperty());
	}
	...
	private Node<OWLClass> nodeToOwlClassNode(ClassNode n) {
		...
		for(IntIterator it = n.getEquivalentConcepts().iterator(); 
				it.hasNext(); ) {
			int eq = it.next();
	...
	private NodeSet<OWLClass> nodesToOwlClassNodeSet(Set<ClassNode> nodes) {
		Set<org.semanticweb.owlapi.reasoner.Node<OWLClass>> temp = 
				new HashSet<>();
