...
final public class Factory implements IFactory {
    ...
    public int getConcept(final String key) {
        ...
        if (null == result) {
            if (conceptIdCounter == concepts.length) {
                ...
                if (Snorocket.DEBUGGING) Snorocket.getLogger().info("concept resize to: " + concepts.length);
            }
    ...
    static public IFactory loadAll(final BufferedReader br) throws IOException, ParseException {
        return loadAll(new LineReader(br));
    }

    static IFactory loadAll(final LineReader reader) throws IOException, ParseException {
        checkVersion(reader);
...
//        reader.readLine();
        ...
        for (int i = 0; i < numConcepts; i++) {
            ...
            if (null == line) {
                throw new AssertionError("EOF reached unexpectedly.  " + (numConcepts-i) + " more concepts expected.");
            }
            ...
            if (id != factory.conceptIdCounter) {
                throw new AssertionError("Out of sequence error.  Expected id = " + factory.conceptIdCounter + " got " + id);
            }
            ...
            // yes, we could just pass isVirtual to the call and avoid this test but there
            // is a not insignificant cost to calling setVirtualConcept unnecessarily. 
            if (isVirtual) {
        ...
        for (int i = 0; i < numRoles; i++) {
            ...
            if (id != factory.roleIdCounter) {
                throw new IllegalStateException("Out of sequence error.  Expected id = " + factory.roleIdCounter + " got " + id);
            }
