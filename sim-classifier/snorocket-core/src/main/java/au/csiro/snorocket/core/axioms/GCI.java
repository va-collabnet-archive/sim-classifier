...
public class GCI extends Inclusion {
			...
			if (concepts.length == 1) {
				...
				for (int i = 0; !result && i < concepts.length; i++) {
					if (!(concepts[i] instanceof Concept)) {
						...
						final AbstractConcept[] newConcepts = 
								new AbstractConcept[concepts.length];
				...
				if (!result) {
					if (concepts.length > 2) {
						...
						result = true;
						final AbstractConcept[] newConcepts = 
								new AbstractConcept[concepts.length - 1];
						...
						throw new AssertionError("Conjunctions of fewer than " +
	...
	public NormalFormGCI getNormalForm() {
		...
		if (lhs instanceof Concept) {
			...
			final AbstractConcept[] concepts = conjunction.getConcepts();
			if (concepts.length == 1) {
				result = NF1a.getInstance(concepts[0].hashCode(), rhs
						.hashCode());
			} else if (concepts.length == 2) {
				result = NF1b.getInstance(concepts[0].hashCode(), concepts[1]
