...
public class OWLImporter {
	...
	public Set<Inclusion> transform(OWLOntology ont, 
        ...
        int totalAxioms = 
    		...
    		ont.getAxiomCount(AxiomType.EQUIVALENT_OBJECT_PROPERTIES, true) +
	...
	private AbstractLiteral getLiteral(OWLLiteral l) {
		...
		if(dt.isBoolean()) {
			...
			switch(odt) {
				...
				case XSD_DATE_TIME:
					res = new DateLiteral(
							DatatypeConverter.parseDateTime(literal));
					break;
				default:
					throw new IllegalArgumentException(
							"Unsupported literal "+l);
			}
		}
                	...
                	if(!pe.isAnonymous()) {
                		...
                		if(dp.equals(odp)) {
                			...
                			if(!compatible) {
                				...
                				problems.add("The literal value restriction "+e+
                		...
                		System.err.println("Found anonymous data property " +
                				"expression in data property range axiom: "+pe);
                	}
                }
